...
class Prestamo(Base):
    ...
    """FK"""
    Id_Bibliotecario = Column(
        UUID(as_uuid=True), ForeignKey("Bibliotecarios.Id_Bibliotecario"), index=True
    )
    Id_Cliente = Column(
        UUID(as_uuid=True), ForeignKey("Clientes.Id_Cliente"), index=True
    )
